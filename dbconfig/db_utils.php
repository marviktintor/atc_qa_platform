...
class db_utils{
	...
	public function __construct(){ 
		...
		$link = new database();	
		$this->db = $link->open_database_connection("atc_qa_platform");
	}
	public function drop_table($tablename,$link){
		//$this->drop_table("test", $this->db);
		/*mysqli_query($link, "drop table `".$tablename."`");*/
	}
	 ...
	 */
	public function is_exists($table,Array $columns,Array  $records){
	...
	/*echo*/  $query .=" ORDER BY `commit_time` DESC;";
	 ...
	 */
	public function delete_record($table,Array $columns,Array  $records){
	...
	public function delete_all_records($table){
		...
		return $this->db->query("DELETE FROM `".$table."` WHERE 1;");
	}
	/**
	 ...
	 * @param array records
	 */
	public function insert_records($table,Array $columns,Array  $records){
		if(empty($table)||count($columns)==0||count($records)==0){
			if(empty($table)){
				throw new NullabilityException("The table name cannot be null");
			}
	 ...
	 * @param array records
	 */
	public function bulk_insert_records($table,Array $columns, Array $records){
		if(empty($table)||count($columns)==0||count($records)==0){
			if(empty($table)){
				throw new NullabilityException("The table name cannot be null");
			}
	...
	public function update_record($table,Array $columns, Array $records, Array $where_columns,Array $where_records){
	 ...
	 */
	public function fetch_assoc($table,Array $columns,Array  $records){
	...
	public function query($table,Array $columns,Array $records){
		return $this->fetch_assoc($table, $columns, $records);
	}
	
	public function search($table,Array $columns,Array $records){
